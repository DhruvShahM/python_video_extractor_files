...
def process_videos():
    ...
    for index, main_video in enumerate(main_videos):
        ...
        half_time_sec_fi = 10
        delay_ms_fi = int(half_time_sec_fi * 1000)
        adelay_value_fi = f"{delay_ms_fi}|{delay_ms_fi}" # Format for adelay e.g., "15000|15000"
        ...
        middle_overlay_end_time_fi = min(half_time_sec_fi + middle_overlay_duration, duration)
        ...
        filter_complex = f"""
        [1:v]scale=iw/3:ih/3[start_v_scaled];
        [2:v]scale=iw/3:ih/3[middle_v_scaled];
        [start_v_scaled]setpts=PTS-STARTPTS+{half_time_sec_fi}/TB[start_v];
        [middle_v_scaled]setpts=PTS-STARTPTS+{half_time_sec}/TB[middle_v];
        [0:v][start_v]overlay=W-w-20:H-h-20:enable='between(t,{half_time_sec_fi},{middle_overlay_end_time_fi})'[tmp1];
        [tmp1][middle_v]overlay=W-w-20:H-h-20:enable='between(t,{half_time_sec},{middle_overlay_end_time})'[vout];
        
        [1:a]atrim=0:{start_overlay_duration},asetpts=PTS-STARTPTS,adelay={adelay_value_fi}|{adelay_value_fi},volume=1.8[start_a];
