...
def replace_audio(video_path, audio_path, output_path):
    try:
        ...
        video.write_videofile(output_path, codec="h264_nvenc", audio_codec="aac", fps=30)  # Set FPS to 30
...
def select_files():
    ...
    video_paths = filedialog.askopenfilenames(title="Select Video Files", filetypes=[("MP4 files", "*.mp4"), ("All files", "*.*")])
    audio_paths = filedialog.askopenfilenames(title="Select Audio Files", filetypes=[("MP3 files", "*.mp3"), ("All files", "*.*")])
    ...
    for name, video_path in video_dict.items():
        if name in audio_dict:
            ...
            output_path = os.path.join(output_folder, f"{name}_output.mp4")
