...
def save_json_file():
    ...
    file_path = filedialog.asksaveasfilename(
        ...
        defaulcodeension=".json",
        filetypes=[("JSON files", "*.json")]
    )
    return file_path

def replace_code_content(old_json_path, new_json_path, output_path):
    """ Updates 'content' in slides where 'slide_type' is 'code' based on matching titles. """
    ...
    # Create a mapping of new content based on title for 'code' slides
    code_content_updates = {
        slide["title"]: slide["content"]
        for slide in new_data if slide.get("slide_type") == "code"
    }

    # Update old slides
    for slide in old_data:
        if slide.get("slide_type") == "code" and slide.get("title") in code_content_updates:
            slide["content"] = code_content_updates[slide["title"]]
...
def main():
    ...
    # Replace content
    replace_code_content(old_json, new_json, output_json)
